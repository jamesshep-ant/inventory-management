...
export default {
  ...
  setup() {
    ...
    const exportCsv = () => {
      ...
      // RFC 4180 CSV escaping: wrap field in double-quotes if it contains a comma,
      // double-quote, or newline; escape embedded double-quotes by doubling them.
      // Without this, fields like "Circuit Boards, Rev 2" would break column alignment.
      ...
      const rows = filteredItems.value.map((item) => {
        const statusKey = getStockStatusKey(item);
      ...
      // Use a temporary <a> element to trigger the browser's file download dialog
